...
if [[ $# -ne 3 ]]; then
  echo "Usage: bash server/run_hanzi_canonical_shugou_checkpoint_experiment.sh REPO EXPECTED_HEAD ARCHIVE_BASE" >&2
  exit 2
fi
if [[ "${HANZI_TEMPORAL_COMPOSITION_AUTHORIZED_RUN:-}" != "canonical-shugou-checkpoint-experiment-v1" ]]; then
  echo "STOP: explicit authorization for the shugou checkpoint experiment is required." >&2
  exit 2
fi
...
CONFIG=configurations/hanzi_stroke_temporal_composition_canonical_shugou_checkpoint_experiment_v1.json
OUTPUT="$REPO/runs/hanzi_stroke_temporal_composition/canonical_shugou_checkpoint_experiment/dev42"
...
PY
  ...
  "$PYTHON" -m hanzi_writing.canonical_shugou_checkpoint_experiment \
    --config "$CONFIG" prepare
  echo "CANONICAL_CHECKPOINT_SELECTION_COMPLETE=1"
  echo "CANONICAL_SHUGOU_CONTROL_STARTED=1"
  echo "CANONICAL_SHUGOU_SUBPHASE_EQUAL_STARTED=1"
  ...
  for arm in control subphase_equal; do
    "$PYTHON" -m hanzi_writing.canonical_shugou_checkpoint_experiment \
      --config "$CONFIG" train-arm --arm "$arm" \
      > "$OUTPUT/worker_logs/$arm.log" 2>&1 &
    WORKER_PIDS+=("$!")
    WORKER_LABELS+=("$arm")
    echo "CANONICAL_SHUGOU_ARM_LAUNCHED=$arm PID=$!"
  done
  test "${#WORKER_PIDS[@]}" -eq 2
  WORKER_FAILURE=0
  for index in "${!WORKER_PIDS[@]}"; do
    pid="${WORKER_PIDS[$index]}"
    label="${WORKER_LABELS[$index]}"
    if wait "$pid"; then
      echo "CANONICAL_SHUGOU_ARM_COMPLETE=$label PID=$pid"
    else
      status="$?"
      echo "CANONICAL_SHUGOU_ARM_FAILED=$label PID=$pid EXIT=$status" >&2
      WORKER_FAILURE=1
    fi
  done
  trap - INT TERM EXIT
...
if [[ "$PROGRAM_EXIT" -ne 0 || "$TEE_EXIT" -ne 0 ]]; then
  echo "Shugou checkpoint experiment stopped; see $LOG_TMP and worker logs." >&2
  exit 1
fi
...
assert provenance["variant"] == "canonical_shugou_checkpoint_experiment_v1"
...
echo "CANONICAL_SHUGOU_CHECKPOINT_EXPERIMENT_COMPLETE=1"
