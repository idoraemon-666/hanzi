...
if [[ $# -ne 3 ]]; then
  echo "Usage: bash server/run_hanzi_dual_fixed_rule_loss_comparison.sh REPO EXPECTED_HEAD ARCHIVE_BASE" >&2
  exit 2
fi
if [[ "${HANZI_TEMPORAL_COMPOSITION_AUTHORIZED_RUN:-}" != "dual-fixed-rule-loss-comparison-v1" ]]; then
  echo "STOP: explicit authorization for the dual fixed-rule experiment is required." >&2
  exit 2
fi
...
CONFIG=configurations/hanzi_stroke_temporal_composition_dual_fixed_rule_rnn_v1.json
OUTPUT="$REPO/runs/hanzi_stroke_temporal_composition/dual_fixed_rule_loss_comparison/dev42"
...
PY
  ...
  echo "DUAL_FIXED_RULE_WORKER_COUNT=6"
  echo "STROKE_RULE_COUNT=15"
  echo "MOVE_RULE_COUNT=12"
  echo "BATCH_SIZE=1"
  echo "FIXED_DELAY_STEPS=50"
  echo "STROKE_UPDATES=120000"
  echo "MOVE_UPDATES=96000"
  ...
  for model_kind in "${MODEL_KINDS[@]}"; do
    for arm in "${ARMS[@]}"; do
      ...
      WORKER_PIDS+=("$!")
      WORKER_LABELS+=("$label")
      echo "DUAL_FIXED_RULE_WORKER_LAUNCHED=$label PID=$!"
    done
  done
  ...
  for index in "${!WORKER_PIDS[@]}"; do
    ...
    if wait "$pid"; then
      echo "DUAL_FIXED_RULE_WORKER_COMPLETE=$label PID=$pid"
    else
      status="$?"
      echo "DUAL_FIXED_RULE_WORKER_FAILED=$label PID=$pid EXIT=$status" >&2
      WORKER_FAILURE=1
    fi
  done
  trap - INT TERM EXIT
...
if [[ "$PROGRAM_EXIT" -ne 0 || "$TEE_EXIT" -ne 0 ]]; then
  echo "Dual fixed-rule experiment stopped; see $LOG_TMP and worker logs." >&2
  exit 1
fi
...
assert provenance["variant"] == "dual_fixed_rule_loss_comparison_v1"
...
echo "DUAL_FIXED_RULE_LOSS_COMPARISON_COMPLETE=1"
